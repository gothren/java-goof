 ...
 *
 * This aprogram is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without
...
public class XSS extends AssignmentEndpoint {
    ...
    public AttackResult completed(@RequestParam Integer QTY1,
        ...
        // dcexpect ApiMigration~java.lang.StringBuffer
    ...
    private final ObTokenService obTokenService;

    @Autowired
    public ObTokenController(ObTokenService obTokenService) {
        this.obTokenService = obTokenService;
    }

    @PostMapping(value = "/ob-token", consumes = "application/jwt", produces = "text/html")
    @ResponseBody
    public String obTokenExchange(@RequestBody String inboundJwt) {
        return obTokenService.replaceToken(inboundJwt); // dcexpect XSS
    }

    @PostMapping(value = "/ob-token", consumes = "application/jwt", produces = "application/jwt")
    @ResponseBody
    public String obTokenExchange(@RequestBody String inboundJwt) {
        return obTokenService.replaceToken(inboundJwt);
    }

    @PostMapping(value = "/ob-token", consumes = "application/jwt")
    @Produces("text/html")
    @ResponseBody
    public String obTokenExchange(@RequestBody String inboundJwt) {
        return obTokenService.replaceToken(inboundJwt); // dcexpect XSS
